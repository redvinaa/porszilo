...
#include "porszilo_sim_interface/porszilo_sim_interface.h"
namespace
{
...
namespace gazebo_ros_control
{
    ...
    bool PorsziloRobotHWSim::initSim(
            ...
            std::vector<transmission_interface::TransmissionInfo> transmissions)
    {
        ...
        for(unsigned int j=0; j < n_dof_; j++)
        {
            ...
            // Debug
            ROS_DEBUG_STREAM_NAMED("porszilo_hw_sim","Loading joint '" << joint_names_[j]
...
//ROS_ERROR("LIMIT on: %s     UP: %f    DOWN: %f",joint_names_[j].c_str(),joint_lower_limits_[j],joint_upper_limits_[j]);
    ...
    void PorsziloRobotHWSim::readSim(ros::Time time, ros::Duration period)
    {
        for(unsigned int j=0; j < n_dof_; j++)
        {
    ...
    void PorsziloRobotHWSim::writeSim(ros::Time time, ros::Duration period)
    {
...
//sim_joints_[j]->SetVelocityLimit(0,joint_velocity_command_[j]);
    ...
    void PorsziloRobotHWSim::eStopActive(const bool active)
    {
...
// Return the joint's type, lower position limit, upper position limit, and effort limit.
    void PorsziloRobotHWSim::registerJointLimits(const std::string& joint_name,
    ...
    PLUGINLIB_EXPORT_CLASS(gazebo_ros_control::PorsziloRobotHWSim, gazebo_ros_control::RobotHWSim)
}
